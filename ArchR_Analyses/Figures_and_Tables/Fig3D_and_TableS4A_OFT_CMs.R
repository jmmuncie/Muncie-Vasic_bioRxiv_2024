...
###----------------Pairwise Marker Features CMs OFT KO v WT----------------------
...
#Now we want to find matches between our WT OFT DARs and the linked peaks:
#'findOverlaps()' will identify matching Granges 
match_WT_DARs_P2G <- findOverlaps(markerpeaks_sorted_OFT_WT, linked_peaks)
#Suset both lists for matches
markerpeaks_sorted_OFT_WT_matched <- markerpeaks_sorted_OFT_WT[match_WT_DARs_P2G@from]
linkedpeaks_matched_OFT_WT <- linked_peaks[match_WT_DARs_P2G@to]
...
pgr <- getPeakSet(proj_Mef2c_v13_E85_E9_subset)
sub_pgr_WT_OFT_DARs_linked_indexes <- findOverlaps(linkedpeaks_matched_OFT_WT, pgr)
sub_pgr_WT_OFT_DARs_linked <- pgr[sub_pgr_WT_OFT_DARs_linked_indexes@to]
...
#Now let's add all the relevant metadata to the linkedpeaks_matched_OFT_WT GRanges object
#First let's add the P2G gene names
linkedpeaks_matched_OFT_WT$GeneName <- metadata(p2g_df)$geneSet$name[linkedpeaks_matched_OFT_WT$idxRNA]
#Next, add Log2FC and FDR values from the diff peak testing
linkedpeaks_matched_OFT_WT$Log2FC_DiffPeakTest <- markerpeaks_sorted_OFT_WT_matched$Log2FC
linkedpeaks_matched_OFT_WT$FDR_DiffPeakTest <- markerpeaks_sorted_OFT_WT_matched$FDR
#Add Correlation and FDR values from p2g linkage
linkedpeaks_matched_OFT_WT$Correlation_p2gLink <- P2G_WT_DARs_df$Correlation
linkedpeaks_matched_OFT_WT$FDR_p2gLink <- P2G_WT_DARs_df$FDR
#Finally, add peakType and nearestGene/TSS info from sub_pgr
linkedpeaks_matched_OFT_WT$peakType <- sub_pgr_WT_OFT_DARs_linked$peakType
linkedpeaks_matched_OFT_WT$nearestGene <- sub_pgr_WT_OFT_DARs_linked$nearestGene
linkedpeaks_matched_OFT_WT$distToGeneStart <- sub_pgr_WT_OFT_DARs_linked$distToGeneStart
linkedpeaks_matched_OFT_WT$nearestTSS <- sub_pgr_WT_OFT_DARs_linked$nearestTSS
linkedpeaks_matched_OFT_WT$distToTSS <- sub_pgr_WT_OFT_DARs_linked$distToTSS

#Create data frame for all WT DARs with P2G link
WT_DARs_P2G_bed_df <- data.frame(
  chr = linkedpeaks_matched_OFT_WT@seqnames,
  start = linkedpeaks_matched_OFT_WT@ranges@start,
  end = linkedpeaks_matched_OFT_WT@ranges@start + 500,
  mm10PeakRange = paste0(linkedpeaks_matched_OFT_WT@seqnames,':',linkedpeaks_matched_OFT_WT@ranges@start,'-',linkedpeaks_matched_OFT_WT@ranges@start + 500),
  peakType = linkedpeaks_matched_OFT_WT$peakType,
  peakLog2FC = linkedpeaks_matched_OFT_WT$Log2FC_DiffPeakTest,
  peakFDR = linkedpeaks_matched_OFT_WT$FDR_DiffPeakTest,
  LinkedGene = linkedpeaks_matched_OFT_WT$GeneName, 
  P2G_Correlation = linkedpeaks_matched_OFT_WT$Correlation_p2gLink,
  P2G_FDR = linkedpeaks_matched_OFT_WT$FDR_p2gLink,
  nearestGene = linkedpeaks_matched_OFT_WT$nearestGene,
  distToNearestGeneStart = linkedpeaks_matched_OFT_WT$distToGeneStart,
  nearestTSS = linkedpeaks_matched_OFT_WT$nearestTSS,
  distToNearestTSS = linkedpeaks_matched_OFT_WT$distToTSS
)
...
#Now we want to find matches between our KO OFT DARs and the linked peaks
#(which we already retrieved above):

#'findOverlaps()' will identify matching Granges 
match_KO_DARs_P2G <- findOverlaps(markerpeaks_sorted_OFT_KO, linked_peaks)
#Suset both lists for matches
markerpeaks_sorted_OFT_KO_matched <- markerpeaks_sorted_OFT_KO[match_KO_DARs_P2G@from]
linkedpeaks_matched_OFT_KO <- linked_peaks[match_KO_DARs_P2G@to]
#Note that these two granges above match, only that the markerpeaks object 
#contains the metadata inlcuding log2fc from the differential peak analysis

#Intersect these peaks with the full peak set to get nearestGene and peakType info
#Subset matching peaks
sub_pgr_KO_OFT_DARs_linked_indexes <- findOverlaps(linkedpeaks_matched_OFT_KO, pgr)
sub_pgr_KO_OFT_DARs_linked <- pgr[sub_pgr_KO_OFT_DARs_linked_indexes@to]
...
#Now let's add all the relevant metadata to the linkedpeaks_matched_OFT_KO GRanges object
#First let's add the P2G gene names
linkedpeaks_matched_OFT_KO$GeneName <- metadata(p2g_df)$geneSet$name[linkedpeaks_matched_OFT_KO$idxRNA]
#Next, add Log2FC and FDR values from the diff peak testing
linkedpeaks_matched_OFT_KO$Log2FC_DiffPeakTest <- markerpeaks_sorted_OFT_KO_matched$Log2FC
linkedpeaks_matched_OFT_KO$FDR_DiffPeakTest <- markerpeaks_sorted_OFT_KO_matched$FDR
#Add Correlation and FDR values from p2g linkage
linkedpeaks_matched_OFT_KO$Correlation_p2gLink <- P2G_KO_DARs_df$Correlation
linkedpeaks_matched_OFT_KO$FDR_p2gLink <- P2G_KO_DARs_df$FDR
#Finally, add peakType and nearestGene/TSS info from sub_pgr
linkedpeaks_matched_OFT_KO$peakType <- sub_pgr_KO_OFT_DARs_linked$peakType
linkedpeaks_matched_OFT_KO$nearestGene <- sub_pgr_KO_OFT_DARs_linked$nearestGene
linkedpeaks_matched_OFT_KO$distToGeneStart <- sub_pgr_KO_OFT_DARs_linked$distToGeneStart
linkedpeaks_matched_OFT_KO$nearestTSS <- sub_pgr_KO_OFT_DARs_linked$nearestTSS
linkedpeaks_matched_OFT_KO$distToTSS <- sub_pgr_KO_OFT_DARs_linked$distToTSS

#Create data frame for all KO DARs with P2G link
KO_DARs_P2G_bed_df <- data.frame(
  chr = linkedpeaks_matched_OFT_KO@seqnames,
  start = linkedpeaks_matched_OFT_KO@ranges@start,
  end = linkedpeaks_matched_OFT_KO@ranges@start + 500,
  mm10PeakRange = paste0(linkedpeaks_matched_OFT_KO@seqnames,':',linkedpeaks_matched_OFT_KO@ranges@start,'-',linkedpeaks_matched_OFT_KO@ranges@start + 500),
  peakType = linkedpeaks_matched_OFT_KO$peakType,
  peakLog2FC = linkedpeaks_matched_OFT_KO$Log2FC_DiffPeakTest,
  peakFDR = linkedpeaks_matched_OFT_KO$FDR_DiffPeakTest,
  LinkedGene = linkedpeaks_matched_OFT_KO$GeneName, 
  P2G_Correlation = linkedpeaks_matched_OFT_KO$Correlation_p2gLink,
  P2G_FDR = linkedpeaks_matched_OFT_KO$FDR_p2gLink,
  nearestGene = linkedpeaks_matched_OFT_KO$nearestGene,
  distToNearestGeneStart = linkedpeaks_matched_OFT_KO$distToGeneStart,
  nearestTSS = linkedpeaks_matched_OFT_KO$nearestTSS,
  distToNearestTSS = linkedpeaks_matched_OFT_KO$distToTSS
)
...
DEGs <- FindMarkers(mef2c_v13_E85_v3_harmony_subset, asay = "SCT", ident.1 = "KO", ident.2 = "WT", group.by = "Genotype", subset.ident = "CMs-OFT")
...
#Save dataframe to a CSV file - Table S4A
write.csv(DARs_P2G_with_DEG_info, "~/Desktop/Working_Directory/TableS4A_E85_OFT-CMs_DARs_P2G_with_DEG_info.csv", row.names = FALSE)
...
#Add manual labels
Wnt11_indices <- which(DARs_P2G_with_DEG_info[, 8] == "Wnt11")
first_Wnt11 <- Wnt11_indices[1]
Myl3_indices <- which(DARs_P2G_with_DEG_info[, 8] == "Myl3")
first_Myl3 <- Myl3_indices[1]
Tdgf1_indices <- which(DARs_P2G_with_DEG_info[, 8] == "Tdgf1")
first_Tdgf1 <- Tdgf1_indices[1]
Mybpc3_indices <- which(DARs_P2G_with_DEG_info[, 8] == "Mybpc3")
first_Mybpc3 <- Mybpc3_indices[1]
...
Mef2c_indices <- which(DARs_P2G_with_DEG_info[, 8] == "Mef2c")
first_Mef2c <- Mef2c_indices[1]
...
man_indices <- c(smallest_x_indices, smallest_unique_y_indices, largest_x_indices, largest_unique_y_indices,
                 first_Wnt11, first_Myl3, first_Tdgf1, first_Mybpc3, first_Nkx25,
                 first_Myh6, first_Myh7, first_Mef2c
)
...
#Add gene name labels only to points to label
...
myplot
pdf("~/Desktop/Working_Directory/P2G_DEG_Correlation_Plots/Fig3D_E85_OFT-CMs_P2G_DEG_corr.pdf", width = 10, height = 8)
print(myplot)
dev.off()
